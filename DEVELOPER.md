...
- You need **command palette**, **keyboard actions**, **shared interactions** as a reusable, package-level boundary.
- You want to consume typed exports from `@platform/command-palette` instead of reaching into app-specific internals.
...
| `framework/libraries/command-palette` | Nested publishable library package. |
| `framework/libraries/command-palette/src` | Public runtime source and exported modules. |
| `framework/libraries/command-palette/tests` | Unit and contract verification where present. |
...
| --- | --- |
| `index.ts` | `packageId`, `packageDisplayName`, `packageDescription`, `PlatformCommandDialog`, `rankCommandPaletteItems`, `filterCommandPaletteItems`, `groupCommandPaletteItems`, `PlatformCommandPalette` |
...
- Hosts should import from `@platform/command-palette`, not deep internal file paths, so refactors inside `src/` do not become accidental breaking changes.
...
```mermaid
graph TD
  host["Host app or plugin"] --> import["Import from @platform/command-palette"]
...
console.log(typeof PlatformCommandDialog);
```
